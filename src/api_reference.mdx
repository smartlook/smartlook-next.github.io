...
XAMARIN: Full SDK setup configuration can be done with optional \`setup\` method parameter.
`}</TextBlock>
...
<TextBlock visibleOn="android,ios,react,flutter,unity,cocos,xamarin">{`
## Blacklisted classes
`}</TextBlock>
...
<TextBlock visibleOn="xamarin">{`
Sometimes it can be beneficial to blacklist all instances of a **\`UIView\` (iOS) or \`View\` (Android) subclass**:
`}</TextBlock>
...
<TextBlock visibleOn="xamarin">{`
When application has some parts displayed using a **WebView** (\`WKWebView\` on iOS, \`WebView\` on Android) and theese parts should be recorded, **WebView** recording needs to be enabled by removing the respective class from list of blacklisted classes:
`}</TextBlock>
